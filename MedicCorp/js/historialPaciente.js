...
document.addEventListener('DOMContentLoaded', function() {
    // Obtener el ID del paciente de la URL (no me sirve tengo que hacer una consulta al servidor para pedir la id del paciente)

    fetch(`http://localhost:5000/consultarPaciente`, {
        method: 'GET',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            document.title = `Historial de ${paciente.nombre_paciente} - MedicCorp`;

            // Cargar los datos del paciente
            cargarDatosPaciente(paciente.id_paciente);
            
            // Cargar las consultas del paciente
            cargarConsultasPaciente(paciente.id_paciente);
